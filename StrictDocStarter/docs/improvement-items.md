...
### Phase 5（2026-06-07, ステークホルダ要求の独立 + UID 全面再採番 / D-9f / strictdoc 0.23.1）
- **課題**: 通しレビューで「ユースケースが要求モデルとして見えない（アクター/UC図/UC一覧なし、 ストーリーが要求グラフ外、 1 ドメイン=1 UC に圧縮）」「UID の欠番・順序乱れが教材の心理的ハードル」が判明。
- **ステークホルダ要求の独立 (D-9f)**: 新規 `01-stakeholder-requirements.sdoc` を新設し、 各ドメインの L0（ステークホルダ要求）を集約。 アクター定義（整備士 / OEMエンジニア / フリート運用者 / OEMバックエンド + 脅威アクター）＋ ユースケース図（Mermaid）＋ UC 一覧（UC→担当ドメイン）を追加。 読み順「00 背景 → 01 最上位（ステークホルダ/UC）→ 02 以降 ドメイン詳細」を確立（ANMS Foundation→Requirements / STFB）。 各ドメイン文書は L1 以下に。
- **ファイル繰り下げ**: 01-auth→02-auth … 09-test-results→10-test-results（90 据置）。 文書名参照（約 40 箇所）を一括更新。
- **UID 全面再採番（欠番・乱れの解消）**: AUTH-L1 を出現順に整列（008→003 等）、 AUTH-L3 / DATA-L3 の欠番を詰め（移設痕跡を解消）、 ARCH-C の 006↔008 スワップ、 API を出現順に整列。 2 段階置換（旧→一時トークン→新）で衝突なく一括変換。 結果、 全カテゴリが連番・欠番ゼロ（API:001-010 / ARCH-C:001-017 / AUTH-L1:001-011 等）。 **教材なのでトレース安定性より見た目の整合を優先**（実務では UID 不変が原則だが、 サンプルは初見の心理的ハードル低減を重視）。 UID はファイル名と独立（StrictDoc は UID 参照）のためリネームでトレース不変。
- **05→06 移設マッピング表の撤去**: 欠番解消により不要・誤情報化したため削除。
- **スペル**: 05-sw-update の「切替 / 切替え」送り仮名を統一。
- **textX 落とし穴（新知見）**: 文書直下 FREETEXT（[TEXT]）と後続 [[SECTION]] の間が **空行 2 つ以上**だと textX が `Expected EOF` で落ちる（awk のセクション削除が余分な空行を残したのが発端）。 **要素間の空行は 1 つに**。
