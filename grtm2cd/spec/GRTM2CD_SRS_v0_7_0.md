...
Let $P_{\text{cat}} = W_{\text{cat}} \times H_{\text{cat}}$ (Original Cat Image pixel count).

Among candidates:

1. Prefer entries where $W_{\text{entry}} \times H_{\text{entry}} \geq P_{\text{cat}}$ (avoid downscaling the Cat Image when possible).
2. Among those, select the entry with the **smallest** pixel count (closest to the original size).
3. If no entry has pixel count $\geq P_{\text{cat}}$, select the entry with the **largest** pixel count (the best available).
...
$$\text{totalCapacityBytes} = \text{availableBytes}(H_{\text{entry}}, W_{\text{entry}})$$

Return $\{ W_{\text{new}}, H_{\text{new}}, \text{totalCapacityBytes}, \text{label} \}$.

If $W_{\text{new}} \times H_{\text{new}} > 16{,}777{,}216$, raise `ERR_PAYLOAD_TOO_LARGE`.

#### 4.2.4 Carrier Resize Policy

Both the Cat Image and the Dog Image are **stretched** to the selected standard resolution $W_{\text{new}} \times H_{\text{new}}$. This may change the aspect ratio of both images. The system displays a warning if either image's original aspect ratio differs from the selected standard resolution's aspect ratio (ADR-26, ADR-24).
...
Flow:
  ...
  6. System displays the selected standard resolution label and dimensions.
...
sequenceDiagram
    ...
    Cap -->> Encode : return {newWidth, newHeight, totalCapacityBytes, label}
    ...
    Encode -->> Main : return {catStegoPixels, dogStegoPixels, width, height, label}
...
Note: `SNAP_UNIT` and `MAX_CARRIER_DIMENSION` have been removed (ADR-26). `STANDARD_RESOLUTIONS` is imported from `standardResolutions.js` (SRP, SoC).
...
import { selectResolution } from "../domain/capacityCalc.js";
...
export async function execute({
  ...
  const { newWidth, newHeight, totalCapacityBytes, label } = selectResolution(
    compressedTM.length,
...
| `downscaleToLimit` | `(fileBytes: Uint8Array, maxPixels: number) → Promise<{pixels: Uint8Array, width: number, height: number, downscaled: boolean}>` | Valid image                 | Downscales if exceeding limit, preserving aspect ratio. No-op if within limit. | —                        |
...
| Condition                                                               | Message                                                                   |
| ----------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| Cat or Dog Image aspect ratio differs from selected standard resolution | "Images will be stretched to {label} ({W}×{H})."                          |
