...
# -- hierarchical (nested sub-cluster) PoC constants --
SUB_GAP = 2 * PAD + 46     # horizontal gap between sub-groups (room for inner boxes)
HEADER_GAP = TOP_PAD + 70  # vertical gap: header node (Conception) above the sub-row
OUTER_PAD = PAD + 30       # extra side pad for an OUTER box that wraps inner boxes
OUTER_TOP = TOP_PAD + 26   # extra top pad for an OUTER box label
...
def top_seg(ckey):
    """Top-level cluster segment of a path string ('consider/world' -> 'consider')."""
    return ckey.split("/")[0] if ckey else None
...
def _layout_one_cluster(ckey, members, edges, nid, opt, specs, rankdir):
    ...
    for e in edges:
        if e["source"] in names and e["target"] in names:
            ...
            g.append("%s -> %s;" % (a, b))
            has_internal = True
    # No internal edges -> dot would place members side by side. Chain them with
    # INVISIBLE edges so they stack in member order (e.g. Probe / TurnRecord in the
    # input port). The invisible-edge trick controls placement; nothing is drawn.
...
def _layout_hier_cluster(ckey, members, edges, nid, opt, specs):
    """Lay out a cluster whose members carry SUB-PATHS (e.g. 'consider/world') as a
    nested grid -- the Variant A core of the cluster-hierarchy feature.

    Each sub-group is laid out INDEPENDENTLY (its own dot run, TB column -- dot is
    reliable on a single simple group) and the sub-groups are composed LEFT->RIGHT
    in first-appearance order BY PYTHON (so the order is guaranteed, sidestepping
    dot's unreliable sibling-cluster ordering). Any member whose cluster == ckey
    exactly (no sub-path) is a HEADER, placed centred ABOVE the sub-group row.

    Returns (pos, {}, (w, h)) exactly like _layout_one_cluster, so the OUTER banded
    composition treats this cluster as one cell and is UNCHANGED. Routes come from
    the final pinned pass over the whole graph, so {} is returned here.
    """
    header = [n for n in members if n.get("cluster") == ckey]
    subs = OrderedDict()
    for n in members:
        c = n.get("cluster")
        if c != ckey:
    ...
    laid = OrderedDict()    # subkey -> (pos, routes, (w,h))
    for sub, snodes in subs.items():
        laid[sub] = _layout_one_cluster(sub, snodes, edges, nid, opt, specs, "TB")

    subs_w = sum(laid[s][2][0] for s in laid) + SUB_GAP * max(0, len(laid) - 1)
    header_w = max((node_size(n, opt)[0] for n in header), default=0.0)
    header_h = max((node_size(n, opt)[1] for n in header), default=0.0)
    total_w = max(subs_w, header_w)

    pos = {}
    sub_y = (header_h + HEADER_GAP) if header else 0.0
    x = (total_w - subs_w) / 2.0           # centre the sub-row under the header
    for sub in laid:
        spos, _, (sw, sh) = laid[sub]
        for n, (nx, ny) in spos.items():
            pos[n] = (nx + x, ny + sub_y)
        x += sw + SUB_GAP
    for n in header:                       # header node(s) centred above the row
        w, h = node_size(n, opt)
        pos[nid[n["name"]]] = ((total_w - w) / 2.0, 0.0)

    W = total_w
    H = sub_y + max((laid[s][2][1] for s in laid), default=0.0)
    return pos, {}, (W, H)
...
def _dot_layout_banded(nodes, edges, nid, opt, rows):
    ...
    for n in nodes:
        by_cluster.setdefault(top_seg(n.get("cluster")), []).append(n)   # group by TOP segment
    specs = opt.get("clusters", {})

    # 1. lay out every (top-level) cluster independently. A cluster whose members
    #    carry sub-paths ('consider/world') is laid out as a NESTED grid; a cluster
    #    that is the SOLE member of its row spans the full width (LR wide strip);
    #    others sharing a row are laid TB (tall column reading top-down).
    solo = {row[0] for row in rows if len(row) == 1}
    laid = {}   # ckey -> (pos, routes, (w,h))
    for ckey, members in by_cluster.items():
        if ckey is None or not members:
            continue
        if any(m.get("cluster") != ckey for m in members):   # has sub-paths -> nested
            laid[ckey] = _layout_hier_cluster(ckey, members, edges, nid, opt, specs)
        else:
...
def cluster_box_cells(nodes, nid, pos, opt):
    ...
    specs = opt.get("clusters", {})
    # group members both by FULL path (inner sub-boxes) and by TOP segment (outer box)
    by_full, by_top = OrderedDict(), OrderedDict()
    for n in nodes:
        c = n.get("cluster")
        if c is None:
            continue
        by_full.setdefault(c, []).append(n)
        by_top.setdefault(top_seg(c), []).append(n)

    def box(ckey, members, pad, top_pad):
        xs0, ys0, xs1, ys1 = [], [], [], []
        for n in members:
            w, h = node_size(n, opt)
            x, y = pos[nid[n["name"]]]
        ...
        return cell, (bx + bw, by + bh)

    cells, maxx, maxy = [], 0, 0
    inner_tops = {top_seg(c) for c in by_full if "/" in c}   # tops that contain sub-boxes
    # OUTER boxes first (document order = z-order -> drawn behind the inner boxes).
    # A top cluster that wraps inner sub-boxes gets extra pad so they nest with margin.
    for tkey, members in by_top.items():
        pad, top_pad = (OUTER_PAD, OUTER_TOP) if tkey in inner_tops else (PAD, TOP_PAD)
        cell, (ex, ey) = box(tkey, members, pad, top_pad)
        cells.append(cell); maxx, maxy = max(maxx, ex), max(maxy, ey)
    # INNER boxes (only nested paths, i.e. those containing "/")
    for fkey, members in by_full.items():
        if "/" not in fkey:
            continue
        cell, (ex, ey) = box(fkey, members, PAD, TOP_PAD)
        cells.append(cell); maxx, maxy = max(maxx, ex), max(maxy, ey)
