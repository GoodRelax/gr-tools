...
**Result:** PASS  **Remark:** `table` シナリオ(SC-101〜107)は `tests/test_table.py`(14 ケース)、`draw` シナリオ(SC-001〜016)は `tests/test_draw.py`(15 ケース)で検証済み(全 PASS)。SC-011 は欠番(0.2.2 でエイリアス受入シナリオを削除)。SC-012(整形式でない XML)は `esc()` により通常到達不能のため fault injection 前提で skip。
