# -*- coding: utf-8 -*-
"""Single source of truth (SSOT) for the LKAS spec.

Every output format (txt/md/sdoc/drawio/html/xlsx/docx/pptx) is rendered from
the structures defined here so that the *logical content* is identical across
formats. Only the figure-encoding (ASCII / mermaid / SVG / mxGraph / shapes)
and the markup differ between formats.

All identifiers and comments are ASCII (English); the document payload itself is
Japanese because the deliverable is a Japanese technical specification.
"""
...
# --------------------------------------------------------------------------
DOC = {
    "title": "車線維持支援システム (LKAS) ソフトウェア仕様検討書",
    "doc_id": "LKAS-SWSPEC-001",
    "version": "0.1",
    "date": "2026-06-19",
    "owner": "車両制御ソフトウェア開発部",
    "abstract": (
        "本書は、車線維持支援システム LKAS の車載ソフトウェアに関する仕様検討書である。"
        "システム概要、機能要求、状態遷移、主要シーケンス、1制御周期の処理フロー、"
        "機能安全に関する考慮、主要パラメータおよび外部インタフェースを示す。"
    ),
}

# --------------------------------------------------------------------------
# Body text. Each section: (number, heading, [paragraphs]).
# Paragraphs are stored verbatim (no hard wrapping) so the same characters
# appear in every format and the content self-check can match them exactly.
# --------------------------------------------------------------------------
SECTIONS = [
    (
        "1",
...
# Tables
# --------------------------------------------------------------------------
PARAM_TABLE_CAPTION = "表1 主要パラメータ"
PARAM_TABLE_HEADER = ["ID", "パラメータ名", "記号", "代表値", "単位", "説明"]
PARAM_TABLE_ROWS = [
...
IF_TABLE_CAPTION = "表2 外部インタフェース"
IF_TABLE_HEADER = ["ID", "信号名", "方向", "接続先", "データ型", "範囲/分解能", "更新周期", "説明"]
IF_TABLE_ROWS = [
...
# Figure captions
# --------------------------------------------------------------------------
FIG_BLOCK = "図1 システム構成図"
FIG_STATE = "図2 状態遷移図"
FIG_SEQ = "図3 主要シーケンス図(車線逸脱検知から操舵介入)"
FIG_FLOW = "図4 処理フローチャート(1制御周期)"
FIG_CAP1 = "図5 LKAS作動状態表示画面(ACTIVE)"
FIG_CAP2 = "図6 車線逸脱警報表示画面"

CAPTURE1 = "capture1.png"
CAPTURE2 = "capture2.png"

# --------------------------------------------------------------------------
# Diagram 1: system block diagram (nodes + directed edges)
# node: (id, label).  edge: (src_id, dst_id, label)
# --------------------------------------------------------------------------
BLOCK_NODES = [
    ("CAM", "前方カメラ"),
    ("SAS", "操舵角センサ"),
...
BLOCK_SENSORS = ["CAM", "SAS", "YRS", "VSS", "TRQ"]
BLOCK_ACTUATORS = ["EPS", "MET", "BUZ"]
BLOCK_EDGES = [
    ("CAM", "ECU", "車線情報"),
    ("SAS", "ECU", "操舵角"),
    ("YRS", "ECU", "ヨーレート"),
...
# state id == label. transition: (src, dst, label)
# --------------------------------------------------------------------------
STATES = ["OFF", "STANDBY", "ACTIVE", "OVERRIDE", "FAULT"]
STATE_INITIAL = "OFF"
STATE_TRANSITIONS = [
    ("OFF", "STANDBY", "システムON"),
    ("STANDBY", "OFF", "システムOFF"),
    ("STANDBY", "ACTIVE", "作動条件成立"),
    ("ACTIVE", "STANDBY", "作動条件不成立"),
    ("ACTIVE", "OVERRIDE", "ドライバ操舵>Tovr"),
    ("OVERRIDE", "ACTIVE", "ドライバ操舵解除"),
    ("ACTIVE", "FAULT", "故障検出"),
    ("STANDBY", "FAULT", "故障検出"),
    ("OVERRIDE", "FAULT", "故障検出"),
    ("FAULT", "OFF", "リセット/IG OFF"),
]

# --------------------------------------------------------------------------
# Diagram 3: sequence (departure detection -> steering intervention)
# participant: (id, label).  message: (src_id, dst_id, label)
# self-message uses src == dst.
# --------------------------------------------------------------------------
SEQ_PARTICIPANTS = [
    ("CAM", "前方カメラ"),
    ("ECU", "LKAS ECU"),
...
SEQ_MESSAGES = [
    ("CAM", "ECU", "車線情報(横偏差,曲率,信頼度)"),
    ("ECU", "ECU", "横偏差/TLC算出"),
    ("ECU", "ECU", "逸脱判定(TLC<TLCth)"),
    ("ECU", "HMI", "逸脱警報要求"),
    ("ECU", "ECU", "目標補助トルク算出"),
    ("ECU", "EPS", "補助操舵トルク指令"),
    ("EPS", "DRV", "操舵トルク付与"),
    ("DRV", "ECU", "ドライバ操舵(オーバーライド時)"),
    ("ECU", "EPS", "トルク漸減/介入中断"),
]

# --------------------------------------------------------------------------
# Diagram 4: control-cycle flowchart
# node: (id, kind, label) kind in {start, end, process, decision}
# edge: (src, dst, label)  label "" for unconditional
# --------------------------------------------------------------------------
FLOW_NODES = [
    ("S", "start", "周期開始 (Tc=20ms)"),
    ("P1", "process", "入力取得(センサ/カメラ)"),
    ("P2", "process", "信号妥当性診断"),
    ("D1", "decision", "故障あり?"),
    ("F1", "process", "フェールセーフ(トルク漸減/FAULT)"),
    ("P3", "process", "車線推定(横偏差/ヨー角/曲率)"),
    ("D2", "decision", "作動条件成立?"),
    ("P4", "process", "制御停止(STANDBY出力)"),
    ("P5", "process", "逸脱判定/目標トルク算出"),
    ("D3", "decision", "オーバーライド?"),
    ("P6", "process", "介入抑制(OVERRIDE)"),
    ("P7", "process", "補助トルク指令確定"),
    ("P8", "process", "出力更新(EPS/HMI)"),
    ("P9", "process", "自己診断/WDT更新"),
    ("E", "end", "周期終了"),
]
FLOW_EDGES = [
    ("S", "P1", ""),
...
# --------------------------------------------------------------------------
# Helpers to flatten content for the self-check (logical content strings).
# --------------------------------------------------------------------------
def all_body_paragraphs():
    out = []
    for _num, _head, paras in SECTIONS:
        out.extend(paras)
    return out


def body_char_count():
    return sum(len(p) for p in all_body_paragraphs())


def node_label(diagram_nodes, node_id):
    """Return a node's label. Works for both (id, label) and (id, kind, label)."""
    for t in diagram_nodes:
        if t[0] == node_id:
            return t[-1]
...
if __name__ == "__main__":
    print("body paragraphs:", len(all_body_paragraphs()))
    print("body char count (JP):", body_char_count())
    print("param rows:", len(PARAM_TABLE_ROWS), "interface rows:", len(IF_TABLE_ROWS))
